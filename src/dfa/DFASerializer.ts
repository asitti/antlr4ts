...
export class DFASerializer {
  ...
  toString(): string {
    ...
    if (this.dfa.states) {
      ...
      for (let s of states) {
        ...
        let edgeKeys = [...Array.from(edges.keys())].sort((a, b) => a - b);
        let contextEdges: Map<number, DFAState> = s.getContextEdgeMap();
        let contextEdgeKeys = [...Array.from(contextEdges.keys())].sort((a, b) => a - b);
  ...
  getStateString(s: DFAState): string {
    ...
    if (s.isContextSensitive) {
      stateStr += "*";
      for (let config of Array.from(asIterable(s.configs))) {
